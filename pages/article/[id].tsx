...
const sectionVariants: Variants = {
  ...
  visible: {
    ...
    transition: { 
      duration: 0.5, 
      ease: 'easeOut' as const, // Explicitly type ease as a valid Framer Motion easing
    },
  },
};
...
export default function ArticleDetail({ article, error, readingTime, keywords }: ArticleDetailProps) {
  if (error || !article) {
    return (
      <motion.div
        ...
        className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 bg-gray-100 dark:bg-gray-900 min-h-screen w-full"
      >
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 sm:p-8">
        <motion.div variants={sectionVariants} className="inline-block">
          <Link
            href="/"
            className="text-blue-500 dark:text-blue-300 hover:underline font-medium mb-6"
            aria-label="Return to home page"
          >
            Back to Home
          </Link>
